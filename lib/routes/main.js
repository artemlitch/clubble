...
router.get('/', function (req, res, next) {
    async.waterfall([
        //find random club for search bar
        function(next) {
            Club.random(function (err, randomClub) {
                if (randomClub) next(err, randomClub.name);
                next(err, "Music Club");
            });
        },
        //find new clubs
        function(randomClub, next) {
            Club.find().sort({_id:-1}).limit(10).exec(function (err, newClubs) {
                next(err, randomClub, newClubs);
            });
        },
        //find popular clubs TODO: what is popular?
        function(randomClub, newClubs, next) {
           next(null, randomClub, newClubs, {}); 
        },
        //find clubs linked to user
        function(randomClub, newClubs, popularClubs, next) {
            let context = {
                my_clubs: {}, 
                search_club: randomClub,
