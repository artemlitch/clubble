    ...
    Club.findOne({'clubLink': req.params.clubLink}, (err, club) => {
        ...
        if (req.user) {
            if (club.userIsMember(req.user.dbUser)) {
                club.members.pull(req.user.dbUser);
                req.user.dbUser.memberClubs.pull(club);
                async.parallel({
                    saveClub: done => {
                        club.save((err) => {
                            return done(err);
...
router.delete('/:clubLink', attachDBUser, function (req, res, next) {
    if (!req.user.dbUser.superUser || !req.params.clubLink) {
        return res.end();
    }
    Club.remove({ clubLink: req.params.clubLink }, function(err) {
        if (err) {
            return next(err);
        } else {
